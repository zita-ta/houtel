...
<body>
    ...
    <div class="container">
        <div style="text-align: center; margin-bottom: 50px;">
            <h1 style="color: var(--dark-brown);">Our Luxury Hotels</h1>
            <p style="color: var(--warm-brown); font-size: 1.2rem; max-width: 600px; margin: 20px auto;">
                Discover our collection of calm cream-themed luxury hotels
            </p>
        </div>
        
        <div class="hotel-grid">
            <?php
            $sql = "SELECT * FROM Hotel ORDER BY nama ASC";
            ...
            <div class="hotel-card">
                ...
                <div class="hotel-card-content">
                    ...
                    <?php else: ?>
                        <a href="login.php" class="cream-btn">
            ...
            <div class="cream-card" style="grid-column: 1 / -1; text-align: center; padding: 60px;">
                ...
                <p style="color: var(--warm-brown);">Check back soon for new luxury hotels.</p>
            </div>
            <?php endif; ?>
        </div>
    </div>
    ...
    window.addEventListener('DOMContentLoaded',()=>{
        <?php if(!isset($_SESSION['user_id'])): ?>
        // Intercept login-to-book buttons
        document.querySelectorAll('a[href="login.php"]').forEach(btn=>{
            btn.addEventListener('click',function(e){
                e.preventDefault();
                showToast('warning','Kamu harus login dulu untuk booking hotel!');
                setTimeout(()=>window.location.href='login.php',1800);
            });
        });
        <?php else: ?>
        // Intercept book now buttons for logged-in users
        document.querySelectorAll('a[href*="booking.php"]').forEach(btn=>{
            btn.addEventListener('click',function(e){
                e.preventDefault();
                const href=this.href;
                const hotelName=this.closest('.hotel-card').querySelector('h3').textContent;
                showConfirmToast('Konfirmasi Booking',`Lanjutkan booking <strong>${hotelName}</strong>?`,()=>window.location.href=href);
            });
        });
        <?php endif; ?>
    });
